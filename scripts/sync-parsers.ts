...
// @ts-ignore - jison doesn't have types
...
async function syncParsers(options: SyncOptions): Promise<void> {
  ...
  for (const diagramName of subset) {
    ...
    if (!jisonPath) {
      results.push({ name: diagramName, success: false, error: `Unknown diagram type: ${diagramName}` });
      continue;
    }
    ...
    if (!existsSync(sourcePath)) {
      results.push({ name: diagramName, success: false, error: `JISON file not found: ${sourcePath}` });
      continue;
    }
  ...
  writeFileSync(
    join(VENDORED_DIR, "sync-info.json"),
